...
async def on_message(message):
	...
	if message.content.startswith('$length'):
		...
		try:
			...
			result = db.execute("SELECT joined, AGE(joined) FROM members WHERE LOWER(username) = LOWER('{username}')".format(username=username)).fetchone()
	...
	if message.content.startswith('$updatename'):
		...
		if len(result) == 2:
			...
			if db.execute("SELECT EXISTS(SELECT 1 FROM members WHERE LOWER(username) = LOWER('{oldname}'))".format(oldname=oldname)).fetchone()[0] == True:
				#query to update the name
				result = db.execute("UPDATE members SET username = '{newname}' WHERE LOWER(username) = LOWER('{oldname}')".format(newname=newname, oldname=oldname))
